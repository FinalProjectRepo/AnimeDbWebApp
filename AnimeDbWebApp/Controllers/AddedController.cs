...
namespace AnimeDbWebApp.Controllers
{
	...
	public class AddedController(IAddedService service) : Controller
	{
		...
		public async Task<IActionResult> AddedAnime(int page = 1, int itemsPerPage = 50, int status = 0)
		{
			...
			var model = await _service.GetAdded<Anime, AddedAnimeViewModel, AppUserAnime>(userId!, page, itemsPerPage, status);
            return View(model);
		}
        ...
        {
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            var model = await _service.GetAdded<Manga, AddedMangaViewModel, AppUserManga>(userId!, page, itemsPerPage, status);
            return View(model);
        }
    }
}
